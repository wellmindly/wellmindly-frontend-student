...
### Current status (Phase 2R reviewed & accepted — review pass 5, 2026-08-25)
...
**One item is outstanding and it is a report, not code: [B-036](../tasks/BUGS.md).** T-210 §3 required a measured 375/390 walk — `document.documentElement.scrollWidth`/`clientWidth` as numbers, the gutter measured, 11 surfaces walked, desktop rechecked at 1024/1440. The builder's report has none of it. **I cannot take those measurements from here: the in-app browser preview is not enabled on this install** (a `runtimeExecutable` config fails because port 5173 is already held by a foreign `node.exe`, and a `url`-only attach config is rejected outright). Everything statically checkable in that card, I checked, and it holds. T-210 stays `DONE` rather than `ACCEPTED` until the numbers land.
...
**Next for me:** author **Phase 4 (dashboard home)** once B-036 closes — T-401 `OverviewTab` redesign, T-402 the mood mosaic that silently calls `onDailyCheckin(3)`, T-403 `WellbeingChart`'s empty Jan–Jun axis, T-404 the three divergent quick-action card styles, T-405 `CheckinModal`/`ScreeningModal` onto `Sheet`. Holding until the landing route is measured, so the dashboard cards can assume a mobile baseline that was verified rather than asserted.

**Debt map, whole tree:** 869 guard errors across 81 files — `off-system-palette` 522, `raw-white` 225, `micro-type` 68, `raw-hex` 54. Worst files: `CounselorBookingView` 183, `TalkMindlyTab` 138, `OverviewTab` 72, `AssessmentsTab` 63, `ReportDetailModal` 60, `AssessmentWizard` 53, `ResultView` 46. `pages/Login.tsx` has left the list entirely. Two mechanical substitutions account for 86% of it, which is worth knowing before Phase 10 looks frightening.
...
*Last updated: 2026-08-25. Round completed: **review pass 5 — Phase 2R (landing revision) closed and the pass-4 bug queue verified.** T-208 and T-209 `ACCEPTED`, B-029 → B-035 all `VERIFIED`, zero reopens; care path 500 → 154 lines and 11 props → 3; route gutter 48px → 24px at 375. `nested-gutter` added to `guard.mjs` as an error (0 hits tree-wide, so pure regression prevention). One item outstanding: **B-036**, T-210's measured 375/390 route walk — a report, not a code change, and one I cannot take myself because the in-app browser preview is not enabled here. Next: builder runs the walk; I author Phase 4 (dashboard home) once it closes.*
